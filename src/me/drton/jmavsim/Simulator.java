...
public class Simulator {
	...
	private LatLonAlt myLatLonAlt = new LatLonAlt(0.0, 0.0, 0.0);
	
    public Simulator(String[] arguments) throws IOException, InterruptedException, ParserConfigurationException, SAXException {

        // Create world
        world = new World();
        String[] Args = arguments;
        // Set global reference point
        double myLat = 43.005340;
        double myLon = -78.783272;
        double myAlt = 0.0;
        
        if(Args.length > 4){
            if(Args[4].equals("-startCoords")){
				//System.out.println(Args[5]);
				//System.out.println(Args[6]);
				//System.out.println(Args[7]);
                try{
                    myLat = Double.parseDouble(Args[5]);
                    myLon = Double.parseDouble(Args[6]);
                    myAlt = Double.parseDouble(Args[7]);
                }catch(Exception e){
                    //myLat = 43.005340;
                    //myLon = -78.783272;
                    //myAlt = 0.0;
                    System.out.println("An error has occurred. Default lat/lon/alt values will be used.");
                } 
            } else {
				System.out.println("Arguments were passed, but none were for startCoords.  Using default lat/lon/alt values.");
			}
        } 
        else{
            //myLat = 43.693963;
            //myLon = -79.620472;
            //myAlt = 100.0;
            System.out.println("No user input provided. Default lat/lon/alt values were used");
        }
        
        myLatLonAlt = new LatLonAlt(myLat, myLon, myAlt);
        
        world.setGlobalReference(myLatLonAlt);
        //print lat,lon,alt to see if correct values were set
        System.out.println("Lat: "+myLat+"   Lon: "+myLon+"   Alt: "+myAlt);
		...
		// FIXME -- Need to set a command line flag for showing the default jMAVSim visualization environment.
		//          For now, we're just going to turn it off.
		if (false) {
